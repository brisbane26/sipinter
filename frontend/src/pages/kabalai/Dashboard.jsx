...
export default function Dashboard() {
  ...
  const { ringkasan, progressPerTim, deadlineTerdekat, aktivitas } = data

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-semibold text-gray-900">Dashboard Kabalai</h1>
          ...
          {!deadlineTerdekat?.length ? <EmptyState /> : (
            <div className="space-y-3">
              {deadlineTerdekat.map((t) => {
                ...
                return (
                  <Link key={t.id} to={`/kabalai/tugas/${t.id}`} className="block border-b border-gray-100 last:border-0 pb-3 last:pb-0">
